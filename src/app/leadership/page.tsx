...
const LEADERS = [
  {
    ...
    achievements: [
      "Commerce Graduate, H.R. College of Commerce & Economics, University of Mumbai",
            ...
            {LEADERS.map((leader, li) => (
              <MotionReveal key={leader.name} delay={0}>
                <article className="grid grid-cols-1 lg:grid-cols-[1fr_2fr] gap-16 lg:gap-24">
                  {/* Left: profile */}
                  <div className="leader-profile flex flex-col items-center text-center profile-block">
                    {/* Avatar */}
                    <div
                      className="w-52 h-52 rounded-full overflow-hidden border-2 border-[var(--border)] mb-8 mx-auto"
                      aria-hidden="true"
                    >
                      {leader.photo ? (
                        <Image
                    ...
                    <p className="font-body text-sm text-justify-block text-[var(--text-secondary)] italic font-light leading-relaxed mb-8 text-balance profile-tagline">
                    ...
                    <div className="profile-highlights">
                      ...
                      <ul className="flex flex-col gap-3 feature-list" role="list">
                        {leader.achievements.map((ach) => (
                          <li key={ach} className="flex mobile-icon-stack items-start gap-3">
                  ...
                  {/* Right: biography */}
                  <div className="reading-column flex flex-col gap-8">
                    {leader.biography.map((para, pi) => (
                      <MotionReveal key={pi} delay={0.08 * pi}>
                        <p
                          className="font-body text-base md:text-lg text-justify-block text-[var(--text-secondary)] leading-relaxed font-light"
